...
upc_descr = pd.read_table('nielsen_extracts/HMS/Master_Files/Latest/products.tsv', encoding = "ISO-8859-1")[['upc', 'upc_ver_uc', 'product_group_code', 'product_group_descr', 'size1_amount', 'size1_units']]
...
upc_descr = upc_descr[upc_descr.nb_of_groups_per_upc == 1][['upc', 'upc_ver_uc', 'product_group_code', 'product_group_descr', 'size1_amount', 'size1_units']]
...
# Merging
prices = purchases.merge(trips_loc, on='trip_code_uc')[['trip_code_uc', 'purchase_date', 'is_walmart', 'store_code_uc', 'store_state', 'guessed_store_county', 'guessed_store_county_fips', 'upc', 'upc_ver_uc','product_group_descr', 'upc_price', 'size1_amount', 'size1_units']]
prices['purchase_month'] = prices.purchase_date.dt.month
prices['purchase_year'] = year

# Calculating the price per unit of volume, only for milk
prices = prices[(prices.product_group_descr == 'MILK') & (prices.size1_units == 'OZ')]
prices['price_per_gal'] = prices.upc_price / prices.size1_amount * 128

avg = pd.DataFrame(prices.groupby(['is_walmart', 'store_state', 'guessed_store_county', 'guessed_store_county_fips', 'purchase_year', 'purchase_month', 'product_group_descr']).mean()[['price_per_gal']])

std = pd.DataFrame(prices.groupby(['is_walmart', 'store_state', 'guessed_store_county', 'guessed_store_county_fips', 'purchase_year', 'purchase_month', 'product_group_descr']).std()[['price_per_gal']]).rename(columns={'price_per_gal': 'price_std'})

nb_obs = pd.DataFrame(prices.groupby(['is_walmart', 'store_state', 'guessed_store_county', 'guessed_store_county_fips', 'purchase_year', 'purchase_month', 'product_group_descr']).count()[['price_per_gal']]).rename(columns={'price_per_gal': 'nb_of_obs'})
...
avg_prices.to_csv(f'aggregated_nielsen_milk_{year}.csv')